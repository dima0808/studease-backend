...
public interface SampleMapper {
  ...
  default SampleDto toSampleDto(Sample sample) {
    return SampleDto.builder()
        ...
        .collectionId(sample.getCollection().getId())
        .build();
  }

  default List<SampleDto> toSampleDto(List<Sample> samples) {
    return samples.stream().map(this::toSampleDto).toList();
  }

  default SampleListDto toSampleListDto(List<Sample> samples) {
    return SampleListDto.builder().samples(toSampleDto(samples)).build();
  }
